...
cbp1_r1 <- "../../data_analysis/resampled_data/expon/r1/Cbp1_expon_r1.mean120.sd18.bed" #Cbp1 ChIP-seq replicate 1
cbp1_r2 <- "../../data_analysis/resampled_data/expon/r2/Cbp1_expon_r2.mean120.sd18.bed" #Cbp1 ChIP-seq replicate 2
inp_r1 <- "../../data_analysis/resampled_data/expon/r1/input.expon_r1.mean120.sd18.bed" #Chromatin input replicate 1
inp_r2 <- "../../data_analysis/resampled_data/expon/r2/input.expon_r2.mean120.sd18.bed" #Chromatin input replicate 2
...
macs2 callpeak --outdir ../macs2_Sso/  -n macs2_SsoCbp1_r1 -f BEDPE -g 2992245 -q 0.01 --keep-dup auto --call-summits  -t $cbp1_r1    -c $inp_r1


macs2 callpeak --outdir ../macs2_Sso/  -n macs2_SsoCbp1_r2 -f BEDPE -g 2992245 -q 0.01 --keep-dup auto --call-summits  -t $cbp1_r2    -c $inp_r2


#removing summits within CRISPR arrays
bedtools intersect -v -a ../macs2_Sso/macs2_SsoCbp1_r1_summits.bed -b $crispr_loci > ../macs2_Sso/macs2_SsoCbp1_r1_summits.filt.bed

bedtools intersect -v -a ../macs2_Sso/macs2_SsoCbp1_r2_summits.bed -b $crispr_loci > ../macs2_Sso/macs2_SsoCbp1_r2_summits.filt.bed

#matching peaks for the two replciates based on summits positions
bedtools window -w 40 -a ../macs2_Sso/macs2_SsoCbp1_r1_summits.filt.bed -b ../macs2_Sso/macs2_SsoCbp1_r2_summits.filt.bed > ../macs2_Sso/matchedPeaks.SsoCbp1.w40.bed
```
...
matchedPeaks <- read.table("../macs2_Sso/matchedPeaks.SsoCbp1.w40.bed")

#narrowPeak files with enrichment for peaks in each replicate
narrow.r1 <- read.table("../macs2_Sso/macs2_SsoCbp1_r1_peaks.narrowPeak")
narrow.r2 <- read.table("../macs2_Sso/macs2_SsoCbp1_r2_peaks.narrowPeak")
...
colnames(filteredPeaks2) <- c("nucleotide", "start.r1", "start.r1", "peak.r1", "scoare.r1",
                              ...
                              "enrichment.replicate1", "enrichment.replicate2", "enrichment.average", "start", "end", "peak") #column names for Supplemental File 1

#writing data set with global IDR < 0.01
write.table(filteredPeaks2[,c(1,14:16,13)], "../data/SsoCbp1_pooled_IDRfilteredPeaks_summits.bed", col.names = F, row.names = F, sep="\t", quote = FALSE)

#subset with minimum 5-fold enrichment in peaks (average between two replicates)
write.table(filteredPeaks2[filteredPeaks2$enrichment.average > 5,c(1,14:16,13)], "../data/SsoCbp1_pooled_IDRfilteredPeaks_summits.min5Enr.bed", quote=F, col.names=F, row.names=F, sep="\t")

#write subset with minimum 5-fold enrichment in peaks as Supplemental File 1
write.table(filteredPeaks2[filteredPeaks2$enrichment.average > 5,c(1,14:16,13,11,12)], "../data/SupplementalFile1.S.solfataricusP2.Cbp1_peaks.txt", quote=F, col.names=T, row.names=F, sep="\t")
```
...
filtPeaks.r1 <- read.table("../macs2_Sso/macs2_SsoCbp1_r1_summits.filt.bed")
unmatched.r1 <- narrow.r1[!narrow.r1$V4 %in% matchedPeaks$peak.r1 & narrow.r1$V4 %in% filtPeaks.r1$V4,]
filtPeaks.r2 <- read.table("../macs2_Sso/macs2_SsoCbp1_r2_summits.filt.bed")
...
ggplot2::ggsave("../plots/QC_peakCallingSsoCbp1.pdf", plot = g, device = "pdf", path = NULL,
